...
class HomeView:
    def __init__(self, parent, app):
        tk.Label(parent, text="ĐỒ ÁN TỐT NGHIỆP",
                 font=("Arial", 24, "bold"), bg="white", fg="#2c3e50").pack(pady=10)
        # Đường dẫn ảnh có thể điều chỉnh lại tuỳ cấu trúc project thực tế
        img_path = os.path.join("app.assets", "bach_khoa.jpg")
        try:
            img = Image.open(img_path)
            img = img.resize((600, 300), Image.Resampling.LANCZOS)
            self.home_image = ImageTk.PhotoImage(img)
            tk.Label(parent, image=self.home_image, bg="white").pack(pady=10)
        except Exception as e:
            tk.Label(parent, text=f"Lỗi ảnh: {e}", fg="red", bg="white").pack()

        topic_label = tk.Label(parent,
